		...
		$('#inputText').bind("keypress", 
			 ...
			 {												
				...
				{	
					...
					var real_parent = $('#inputText').parent().parent().parent().attr('id');
					var params = encodeURI("action=insertElement"+"&name="+name+"&real_parent="+real_parent+"&slave="+slave);
...
********************************************************/
	...
	{	
		...
		{
			treeOps.treeBusy = true;
			var real_parent = treeOps.trGetSelected().parent().parent().attr('id');
...
*******************************************************/
	/*this.trUpdateElementName = function(result)
	{
		...
		$('ul.ajax>li.doc-last', '#' + elementId).attr('id', info.elementId).html("{url:"+ structureManagerURL +"?action=getElementList&real_parent="+ info.elementId +"}");
		...
		$('#inputText').bind("keypress",
								 function(evt)
								 {
									 if (evt.keyCode == 13) { //pressed enter
										var name = $('#inputText').attr('value'); 	
										var real_parent = $('#inputText').parent().parent().parent().attr('id');
									 	var params = "action=updateElementName&name="+name+"&elementId="+elementId+"&real_parent="+real_parent;
