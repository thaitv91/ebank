...
where user_id IN (SELECT id_user FROM users where real_parent='$my_id')";
...
?>
	...
	<ul class="pagination">
		...
		{ ?> 
			<li class="prev">
				<a href="<?="index.php?page=downline_gd&p=".($newp-1);?>">&larr; <?=$Previous;?></a>
			</li> <?php  
		}
		for ($i=1; $i<=$pnums; $i++) 
		{ 
			if ($i!=$newp)
			{ ?>
				<li><a href="<?="index.php?page=downline_gd&p=$i";?>"><?php print_r("$i");?></a></li>
				<?php 
			}
			else
		...
		{ ?> 
			<li class="next">
			<a href="<?php echo "index.php?page=downline_gd&p=".($newp+1);?>"><?=$Next;?> &rarr;</a>
			</li> <?php  
		} ?>
	</ul>
	</div>
