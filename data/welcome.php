...
<div class="row" style="z-index:-1;">
    ...
    <div class="col-lg-4 col-xs-12">
        <div class="small-box bg-red">
            <div class="inner"><h3><?=$epin_wal;?></h3><p>vWallet</p></div>
...
while($row = mysql_fetch_array($query))
{
    ...
    $country = $row['country'];
    $address = $row['address'];
    $max_send_value = $row['max_send_value'];
    $sent_value = $row['sent_value'];
}
...
if($max_send_value == -1 ) {
    ...
    if (($max_send_value - $sent_value) > 0) {
        $can_send = 1;
    }
}
...
if(isset($_REQUEST['buy_unit']))
{
    if($max_pd_amt_settings[$level_vtype] >= $_POST['request_amount'] && $can_send == 1)
    {
        ...
        $epin_sql = "select *  from e_pin where mode=1 and user_id='$user_id'";
        $epin_ques = mysql_query($epin_sql);
        $num = mysql_num_rows($epin_ques);
        if($num < $pin_cnt)
        {
            print "<B style=\"color:#FF0000; padding-left:20px;\">Your Ticket wallet is not sufficient. Please buy more Tickets to do a new Sending.</B>";
        }
        else
        {
                ...
                if($num > 0)
                {
                    ...
                    if($epin_tot_chk >= $max_pd_amt_settings[$level_vtype])
                    {
                        ...
                        //$ir_mode != '1' or $ir_mode == NULL) and ($it_mode == '2' or $it_mode == NULL)
                        {
                            ...
                            mysql_query("update users set sent_value = '$sent_value' WHERE id_user = '$user_id'");
...
if(isset($_REQUEST['withdraw']))
{
    ...
    if($num > 0)
    {
		...
		if($wallet_from == 'cwallet'){
            $c_sql = mysql_query("SELECT amount FROM wallet WHERE id = '$user_id'");
            $r_sql = mysql_fetch_array($c_sql);
            $amount_wallet = $r_sql['amount'];
        }
        if($wallet_from == 'rwallet'){
            $c_sql = mysql_query("SELECT com_wallet FROM wallet WHERE id = '$user_id'");
            $r_sql = mysql_fetch_array($c_sql);
            $amount_wallet = $r_sql['com_wallet'];
        }
		
		
		if($wallet_from == 'cwallet' && $request_amount <= 0 OR $wallet_from == 'rwallet' && $request_amount <= 0){
            echo "<B style=\"color:#FF0000;\">Please enter a greater Getting amount!</B>";
        } else if( $request_amount <= $amount_wallet){
						...
						if($request_amount % $amt_multiple == 0)
						{
							if($max_gd_complete_m && $max_gd_complete_w && $max_gd_complete_d){
								...
								else
								{
									...
									$sfl2 = "update wallet set com_wallet = com_wallet-'$request_amount' where id = '$user_id' ";
									mysql_query($sfl);
									mysql_query($sfl2);
								...
								$_SESSION['wid_sucs'] = "<B style=\"color:#008000;\">You Getting request for an amount of ".$request_amount." has been received. You will receive an email when it is processed.</B>";
								...
								echo "<B style=\"color:#FF0000;\">Your Have Exceeded Your Getting Amount Limit According Level !!</B>";
							}
						}
						else
						{
		...
		} else { echo "<B style=\"color:#FF0000;\">Error : Your Wallet Balance is insufficient !!</B>"; }  
    }
    else
    { echo "<B style=\"color:#FF0000;\">Please enter correct code !!</B>"; }
    }else{
        echo "<B style=\"color:#FF0000;\">Your Getting amount must be a multiple of 100,000!</B>";
    }
}
?>
    <div>
                ...
                <div class="widget-body" style="padding-top:15px;">
                    <div class="col-md-6 col-xs-12">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-4 col-xs-12">
                                <input type="submit" name="buy_unit" value="I agree to Send" class="btn btn-info btn-sm" />
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-xs-12">
                        ...
                        <div class="widget-body">
                            <p class="help-block">
                                <label class="control-label" style="color:#FF0000;">
                                    You have exceeded your Sending allowance. Please try again later.
                        </label><br />
                            </p>
                        </div>
                        <?php
                        ...
                        <div class="col-md-6 col-xs-12">
                            <div class="form-group">
                                ...
                                <div class="col-md-4 col-xs-12">
                                    ...
                                    <p class="help-block">
                                        Getting amount : <span id="show_gd_amount">0 = 0</span>
                                    </p>
                                    <p class="help-block red">
                                        <i>Note: GET amount must be a multiple of 100,000</i>
                                    </p>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-6 col-sm-12 col-xs-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4 col-xs-12">
                                            <input type="submit" name="withdraw" value="I agree to Getting" class="btn btn-info btn-sm" />
                                        </div>
                                    </div>
                                </div>
                                <div class="clear-fix"></div>
...
<div class="differance-top">
    ...
    <div class="col-md-8 col-ms-12 col-xs-12">
        <div class="row">
            ...
            <div class="col-md-12 col-sm-12 col-xs-12 link-register">
                <form class="form-inline">
                    <div class="form-group">
                        Please use this link to refer other members to vWallet
                    </div>
                    <?php
                    ...
                    ?>
                    <div class="form-group">
                        <div class="form-control" id="linkRegister">https://vwallet.uk/register.php?ref=<?=$r_linkreg['username'] ?></div>
                    </div>
            ...
            <!-- HOW IT WORK -->
            <div class="how-it-work col-md-12 col-sm-12 col-xs-12">
                <h3 class="blue">HOW VWALLET WORKS?</h3>
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12 step-work step-work-1">
                        <div class="num-step">
                            ...
                            <div class="row">
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <div class="title-step">
                                        <img src="/images/newlayout/icons/step1.png">
                                        +
                                    </div>
                                    <ul class="list-unstyled">
                                        <li>Transfer funds to the Community Wallet and get your ticket. You can do it by directly transferring money to Community Wallet or transferring from Upline.</li>
                                    </ul>
                                </div>
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <img class="img-responsive" src="/images/newlayout/icons/epin-step.png">
                                </div>
                            </div>
                            <div class="clear-fix"></div>
                        </div>
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12 step-work step-work-2">
                        <div class="num-step">
                            <div class="number-step">
                                step 2
                            </div>
                        ...
                        <div class="container-step">
                            <div class="title-step">
                                <img src="/images/newlayout/icons/step2.png">
                                +
                            </div>
                            <ul class="list-unstyled">
                                <li>Place SEND request: Choose the amount of 2,500,000 or 5,000,000; enter the protection password and press the ORDER button.</li>
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12 step-work step-work-3">
                        <div class="num-step">
                            <div class="number-step">
                                step 3
                            </div>
                        ...
                        <div class="container-step">
                            <div class="title-step">
                                <img src="/images/newlayout/icons/step3.png">
                                +
                            </div>
                            <ul class="list-unstyled">
                                <li>Place GET request: select M wallet or C wallet to enable GET, select the amount,  enter the password and press the button to inquire the order.</li>
    ...
    <div class="col-md-4 col-sm-12 col-xs-12">
        <div class="row">
            ...
            <div class="col-md-12 col-sm-12 col-sx-12">
                <div class="widget donate-sidebar pdContainer-<?=$div_class;?>">
                    <div class="donateHead clearfix">
                        <div class="title">Your Sending ID: PH069<?=$id;?></div>
                    </div>
                    <div class="widget-body">
                        <ul class="list-unstyled list-ph">
                            ...
                            <li>
                                ...
                                <b>Sending Amount : <?=number_format($amount);?></b><br>
                            </li>
                            <li>
            ...
            <!-- <div class="col-md-12 col-sm-12 col-xs-12 banner-home">
