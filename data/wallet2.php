...
if($id > 0)
{
	if(isset($_POST['submit']))
	{
		...
		$query = mysql_query("select roi from wallet where id = '$id' ");
		...
		if($request_amount <= $curr_amnt)
		{
			...
			if($num > 0)
			{
				...
				mysql_query("update wallet set roi = roi-'$request_amount',magic_limit = '$request_amount' where id = '$id' ");
	...
	else
	{
		...
		$query = mysql_query("select roi, magic_limit from wallet where id = '$id' ");
		while($row = mysql_fetch_array($query))
		{
			$curr_amnt = $row[0];
			$magic_limit = $row[1];
		...
		$child_invest_chk = chk_child_invest($id ,$other_withdrwal_per);
		$child_invest_chk = $child_invest_chk-$magic_limit;
		?>
		<table id="table-1" class="table table-striped table-hover dataTable" aria-describedby="table-1_info"> 
		<tbody role="alert" aria-live="polite" aria-relevant="all">	
		<form name="money" action="index.php?page=wallet2" method="post">
		<input type="hidden" name="curr_amnt" value="<?php echo $curr_amnt; ?>"  />
		  ...
		  <tr>
			<td class="td_title"  style="color:#FF0000; font-size:14px; font-weight:bold;">Maximum Withdrawal Balance   </td>
			<td class="td_title" style="color:#FF0000; font-size:14px; font-weight:bold;"> <?php echo round($child_invest_chk/$usd_value_current,2)." <font color=DodgerBlue>USD</font> Or  ".$child_invest_chk; ?>  <font color=dark>$ </font></strong></td>
		  </tr>
		  <tr>
			<td colspan="2">&nbsp;</td>   
		  </tr>
...
<?php			for($i = ($setting_inv_amount/1); $i <= $max_withdrawal_amount; $i = $i+($setting_inv_amount/1))
		{ ?>
...
function chk_child_invest($user_id , $withdrwal_per)
{
	$sql = "select sum(t2.amount) as totl_inc from users as t1 inner join daily_interest as t2 on t1.id_user = t2.member_id and  t1.real_parent = '$user_id' ";
