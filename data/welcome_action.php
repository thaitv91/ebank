...
elseif (isset($_POST['Accept']))
{
        ...
        /* Update wallet amount on function get_speed_bonus */
        ...
        if (!empty($idloop))
        {
            ...
            foreach ($manager_parent as $key => $value)
            {
                ...
                if ($level <= 6) {
                    // real level
                    $level_user = $value['level'];
                    if ( $level <= $level_user) {
                        // new level bonus
                        $level_income = find_level_income($level);
                        ...
                        // print data bonus
                        //echo "user: " . $value['username'] . ' - % bonus level: ' . $level . "- Real level: " . $level_user . '- Amount: ' . $manager_bonus . '<br />';   
                    }
                }
            }
        }
  ...
  //       $sponsor_bonus = $income_transfer_amount * $level_income / 100;
