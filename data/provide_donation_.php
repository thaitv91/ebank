			...
			if($num > 0)
			{
				...
				for($i = 0; $i < count($invest_id); $i++)
				{
					...
					while($rojj = mysql_fetch_array($querr))
					{
						...
						$block_time=date('Y-m-d H:i:s',strtotime($times." +".$max_time." hours"));
						
						$tot_second = '';
						if($extend_time != NULL and $extend_time != '0000-00-00 00:00:00')
						{
							$swr = "SELECT TIMESTAMPDIFF(SECOND,'$curr_time','$extnd_time') 
							as seconds";
						...
						else
						{
							...
							/*$ete = strtotime(date("Y-m-d",strtotime($time."+$max_time hours")));
							...
							$swr = "SELECT TIMESTAMPDIFF(SECOND,'$curr_time','$block_time') 
							as seconds";
							$result = mysql_fetch_array(mysql_query($swr));
							$tot_second = $result[0];
						}
							
						$status_tra = getStatusPD($invest_id[$i]);
				...
				<tbody>
					...
					if($num > 0)
					{
						...
						{ 
							...
							$report = report_exist($user_id , $table_id);
							$end_time=date('Y-m-d H:i:s',strtotime($time_link." +".$max_time." hours"));
							
							if($mode != 2)
							{
								if($curr_time > $end_time and $report < 1)
								{
									$report_btn = "<a href=\"#dialog-report-confirm\" data-toggle=\"modal\" class=\"btn btn-danger btn-sm\" id=\"show_report_box_gd_$table_id\" data=\"{&quot;mdid&quot;:&quot;$table_id&quot;,&quot;uid&quot;:&quot;$user_id&quot;,&quot;uir&quot;:&quot;$pay_id&quot;,&quot;invest&quot;:&quot;$inv_id&quot;}\">Report</a>";
								}
								else{ $report_btn = ''; }
	...
	/*
		...
		<div class="widget-body">
			<div class="donateHead clearfix">
				...
				<span class="glyphicon-circle glyphicon-right <?=$bg_class;?>"><?=$inv_profit;?></span>
