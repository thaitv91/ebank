...
if(isset($_POST['submit']))
{
	if($_POST['submit'] == 'Accept')
	{
		...
		mysql_query("update paid_unpaid set paid_date = '$accept_date' , paid = 1 , 
	...
	elseif($_POST['submit'] == 'Cencel')
	{
		...
		$information = $_REQUEST['information'];
		$accept_date= date('Y-m-d');
		mysql_query("update paid_unpaid set paid_date = '$accept_date' , paid = 2 , 
		paid_inform = '$information' where id = '$req_id' ");
...
else
{
	...
	$query = mysql_query("select * from paid_unpaid where paid = 0 and amount > 0 LIMIT $start,$plimit ");
	while($row = mysql_fetch_array($query))
	{
		$id = $row['id'];
		$u_id = $row['user_id'];
		...
		$walletquery = mysql_query("select amount from wallet where id = '$u_id' ");
		while($walletrow = mysql_fetch_array($walletquery))
		{
		...
		if($pay_mode == 'ge_currency')
		{
			...
			<tr>
				...
				<td class="text-center"><?=$wallet_amount;?> <font color=dark>$ </font></td>
...
{
 	$qur = mysql_query("select * from users where id_user = '$id' ");
