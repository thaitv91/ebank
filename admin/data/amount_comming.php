...
<form action="index.php?page=amount_comming" method="post">
...
$que1 = mysql_query("SELECT sum(amount) FROM income_transfer where mode = 0 $qur_set_search");
...
{ ?>
	...
	<?php
	$query = mysql_query("SELECT * FROM income_transfer where mode = 0 $qur_set_search order by id desc");
	...
	$query = mysql_query("SELECT * FROM income_transfer where mode = 0 $qur_set_search order by id desc LIMIT $start,$plimit ");
	...
	{ ?>
		<li id="DataTables_Table_0_previous" class="paginate_button previous">
			<a href="<?="index.php?page=amount_comming&p=".($newp-1);?>">Previous</a>
		</li>
	<?php 
	}
	for ($i=1; $i<=$pnums; $i++) 
	{ 
		...
		{ ?>
			<li class="paginate_button ">
				<a href="<?="index.php?page=amount_comming&p=$i";?>"><?php print_r("$i");?></a>
			</li>
			<?php 
		}
		else
	...
	{ ?>
	   <li id="DataTables_Table_0_next" class="paginate_button next">
			<a href="<?="index.php?page=amount_comming&p=".($newp+1);?>">Next</a>
	   </li>
	<?php 
	} 
	?>
