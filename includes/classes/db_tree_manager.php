        ...
        if ($result !== false)
        {
            ...
            */
            while ($row = $this->db->fetchObject($result))
            {
                ...
                {
					$iiiddd = $row->id_user;
					$queryxcx = mysql_query("SELECT * FROM users WHERE id_user = '$iiiddd' ");
					while($rrrr = mysql_fetch_array($queryxcx))
					{
						$full_name = $rrrr['firstname']." ".$rrrr['lastname']; 
					}
					$queryxcx = mysql_query("select * from users where id_user = $iiiddd ");
					while($rrrr = mysql_fetch_array($queryxcx))
					{
						$user_name = $rrrr['email'];
					}
					$full_name = get_full_name($row->id_user);
					$user_name = get_user_name($row->id_user);
                    $supp = "<ul class='ajax'>"
                ...
                ."<span>".$user_name."</span>"."<span>&nbsp;&nbsp;(".$full_name.")</span>".$supp
                ."</li>";
            }
        }
        return $str;				
