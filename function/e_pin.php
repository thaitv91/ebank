...
function generate_epin($id,$new_user,$product_id,$to,$num,$data_log,$log_type,$from,$SmtpServer,$SmtpPort,$SmtpUser,$SmtpPass,$epin_generate_message,$fees) //$to = email id , num = no of unique id
{
	...
	for($i = 0; $i < $c; $i++)
	{
		$query = mysql_query("select * from e_pin where epin = '$unique_epin[$i]' ");
		...
		if($num == 0)
		{
			...
			$date = date('Y-m-d');
			$t = time();
			mysql_query("insert into e_pin (epin, user_id , product_id , mode , time , date , used_id) values ('$unique_epin[$i]' , '$id' ,'$product_id' , '$mode' , '$t' , '$date' , '$new_user')");
