	...
	if($count >1)
	{
		for($i = 1; $i < $count-1; $i++)
		{
			...
			if($left !=0 or $right !=0)
			{
				...
				$date = ggetdate($id);
				$query = mysql_query("select * from left_right where date = '$date' and user_id = '$parents[$i]' ");
				$num = mysql_num_rows($query);
				//print "nm".$num;
				if($num == 0)
				{
					
					mysql_query("insert into left_right (user_id , left_child , right_child , date) values ('$parents[$i]' , '$left' , '$right' , '$date' ) ");
					
				}
				else
				{
					mysql_query("update left_right set left_child = '$left' , right_child = '$right' where date = '$date' and user_id = '$parents[$i]' ");
				}
			}	
		}
	}	
...
function carry_forward($id)
{
	...
	$query = mysql_query("select * from left_right where user_id = '$id' and date != '$curr_date' ORDER BY id DESC LIMIT 1 ");
	$num = mysql_num_rows($query);
	if($num != 0)
	{
