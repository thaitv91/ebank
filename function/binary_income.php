...
function binary_payment_calculation($income_binary,$income_type)
{
	...
	for($i = 1; $i <= 15; $i++)
	{
		...
		$query = mysql_query("select * from income where date = '$previous_date' and type = 2 ");
	...
	for($i = $count; $i >= 0; $i--)
	{
		$q = mysql_query("select * from left_right where date = '$income_date[$i]' ");
			...
			while($row = mysql_fetch_array($q))
			{
				...
				if($check == 1)
				{
					...
					if($amount != 0)
					{
						...
						mysql_query("insert into income (user_id , amount , date , type , incomed_id) values ('$user_id' , '$deducted_amount' , '$income_date[$i]' , '$income_type[2]' , '$incomed_id') ");
